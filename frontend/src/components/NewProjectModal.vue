  ...
  <div v-if="show" class="fixed inset-0 bg-neutral-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center" @click.self="$emit('close')">
    <div class="relative mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
      <div class="mt-3 text-center">
        <h3 class="text-lg leading-6 font-medium text-neutral-900">Nouveau projet</h3>
        <form @submit.prevent="createProject" class="mt-4 px-7 py-3">
          <input type="text" v-model="projectName" placeholder="Nom du projet" 
                 class="w-full px-3 py-2 text-neutral-700 border rounded-lg focus:outline-none focus:shadow-outline" required>
          <div class="items-center px-4 py-3">
            ...
            <button @click="$emit('close')" type="button" class="mt-2 px-4 py-2 bg-neutral-200 text-neutral-800 rounded hover:bg-neutral-300 w-full">
              Annuler
            </button>
          </div>
        </form>
